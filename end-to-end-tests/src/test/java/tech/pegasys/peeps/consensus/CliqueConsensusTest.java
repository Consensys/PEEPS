...
public class CliqueConsensusTest extends NetworkTest {
  ...
  protected void setUpNetwork(final Network network) {
    alphaNode = network.addNode("alpha", KeyPair.random(), Web3ProviderType.QUORUM);
    private final BesuNode betaNode = network.addNode("beta", KeyPair.random());
    network.set(ConsensusMechanism.CLIQUE, betaNode);
    network.addSigner(signer.id(), signer.resources(), betaNode);
  }
